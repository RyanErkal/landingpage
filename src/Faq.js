import React from "react";

export default function Faq() {
	return (
		<>
			<section class="bg-gray-800 text-gray-100 px-6 py-8">
				<div>
					<h1 class="text-3xl col-span-3 pb-2 justify-self-center subpixel-antialiased">
						<strong class="text-green-400 flex justify-center">
							FAQs
						</strong>
					</h1>
					<div class="flex justify-start mb-4 px-36">
						<p>
							<strong>What is the minimum age to join?</strong>
							<br />
							We only accept clients who are 18 years or older.
						</p>
					</div>
					<div class="flex justify-start mb-4 px-36">
						<p>
							<strong>
								What is the minimum duration of the coaching
								process?
							</strong>
							<br />
							We do not offer coaching for periods under 12 weeks
							at a time, physique progression takes time and due
							to the level of detail and quality of service we
							provide, anything under 12 weeks is insufficient to
							fully benefit from the coaching process.
						</p>
					</div>
					<div class="flex justify-start mb-4 px-36">
						<p>
							<strong>Are results guaranteed?</strong>
							<br />
							Your results are dependent on your adherence to the
							plan, we will provide you with the tools and
							knowledge to achieve your goals, but it is up to you
							to implement them.
						</p>
					</div>
					<div class="flex justify-start mb-4 px-36">
						<p>
							<strong>Do your work with women?</strong>
							<br />
							Yes we work with women, both lifestyle and physique
							competitors.
						</p>
					</div>
					<div class="flex justify-start mb-4 px-36">
						<p>
							<strong>
								Is there a cancellation notice period?
							</strong>
							<br />
							We require a 1 week notice period for cancellations.
						</p>
					</div>
					<div class="flex justify-start mb-4 px-36">
						<p>
							<strong>Is there a refund policy?</strong>
							<br />
							All payments are non-refundable.
						</p>
					</div>
				</div>
			</section>
		</>
	);
}
